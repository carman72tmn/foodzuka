  ...
  <div>
    <VCard class="mb-6">
      <VCardTitle class="d-flex align-center justify-space-between flex-wrap gap-3 pa-4">
        <div>
          <div class="text-h5 font-weight-bold">🍕 Продажи по товарам</div>
          <div class="text-caption text-medium-emphasis">
            Анализ популярности блюд. Источник: iiko OLAP.
          </div>
        </div>
    ...
    <VCard>
      <VDataTable :headers="headers" :items="reportData" :loading="loading" hover class="text-no-wrap">
...
const headers = [
  ...
  { title: 'Количество', key: 'DishAmount', align: 'end' },
...
async function fetchReport(forceRefresh = false) {
  ...
  try {
    ...
    const resp = await fetch(`/api/v1/reports/olap/products?${params}`)
