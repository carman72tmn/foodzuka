          ...
          <VRow>
            ...
            <VCol cols="12" md="4">
              ...
              <VExpansionPanels variant="accordion">
                <VExpansionPanel v-for="(vars, category) in variables" :key="category" :title="category">
      ...
      <VCard rounded="xl">
        ...
        <VCardText class="px-6">
            ...
            <VRow>
              ...
              <VCol cols="12" md="6">
                <VAutocomplete
                  v-model="editingAccount.employee_id"
                  :items="employees"
                  item-title="name"
                  item-value="id"
                  label="Привязать сотрудника"
            ...
            <!-- Типы уведомлений -->
            <VDivider class="my-4" />
            <h4 class="mb-2">Типы уведомлений</h4>
            <VRow>
              <VCol v-for="event in availableEvents" :key="event.type" cols="12" md="6">
                <VCheckbox
                  v-model="editingAccount.enabled_events"
                  :label="event.name"
                  :value="event.type"
                  hide-details
                />
            ...
            <VAlert v-if="editingAccount.enabled_events && editingAccount.enabled_events.length === 0" type="info" variant="tonal" class="mt-2" density="compact">
              Если ничего не выбрано, будут приходить все уведомления
            </VAlert>

          <VTabs density="compact" class="mb-4">
            <VTab @click="newAccount.customerId = null; newAccount.employeeId = null">Обычный</VTab>
        ...
        <VCardText class="px-6">
          ...
          <template v-if="!broadcastData.templateId">
            ...
            <div class="d-flex flex-wrap gap-2">
              <VChip v-for="(vars, category) in variables" :key="category" size="small" @click="insertVariable(Object.keys(vars)[0], 'broadcast')">
                {{ category }}
              </VChip>
            </div>
            <div class="text-caption mt-2 text-grey">Нажмите на категорию, чтобы вставить первую переменную. Для выбора конкретной используйте Редактор Шаблонов.</div>
          </template>
        </VCardText>
        <VCardActions class="pa-6">
