...
import { authState, getAuthHeaders } from '@/utils/auth'
...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Список пользователей">
        <template #append>
          <VBtn
            color="primary"
        ...
        >
          <template #item.username="{ item }">
            <div class="d-flex align-center">
              <VAvatar
                size="32"
                :color="item.role?.code === 'SUPER_ADMIN' ? 'error' : 'primary'"
                variant="tonal"
                class="me-3"
              >
          ...
          <template #item.role_name="{ item }">
            <VChip
              v-if="item.role"
              :color="item.role.code === 'SUPER_ADMIN' ? 'error' : 'info'"
              size="small"
              label
            >
              {{ item.role.name }}
            </VChip>
            <span v-else class="text-disabled">Нет роли</span>
          </template>
