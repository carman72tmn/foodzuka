...
async def test_sales_report():
    ...
    async with httpx.AsyncClient(verify=False, timeout=30) as client:
        ...
        payload = {
            "reportType": "SALES",
            "groupByRowFields": ["OrderNum", "Employee.Name", "OrderType", "OpenTime"],
        ...
        if r2.status_code == 200:
            data = r2.json()
            print(f"SALES rows: {len(data.get('data', []))}")
