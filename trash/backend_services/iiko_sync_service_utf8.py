...
class IikoSyncService:
    ...
    def clean_str(val):
        ...
        # (Обычно это выглядит как РАРАРАРА...)
        try:
    ...
    async def process_iiko_order(self, session: Session, iiko_order_data: Dict[str, Any], organization_id: str, iiko_card_data: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            # Сначала пробуем собрать адрес из наиболее полного объекта (обычно это raw_addr, но если там пусто - raw_addr_dp)
            # Для этого создаем временный объект со всеми полями
            merged_addr_obj = {
                "city": city,
    ...
    async def _send_order_vk_notification(self, order, event_type: str):
        ...
        try:
            ...
            formatted_order = {
                ...
                "expected_time": getattr(order, 'expected_time', '').strftime('%Y-%m-%d %H:%M') if getattr(order, 'expected_time', None) else '---',
    ...
    async def sync_courier_deliveries(self, session: Session, days: int = 14, date_from: Optional[datetime] = None, date_to: Optional[datetime] = None) -> None:
        ...
        for d in deliveries:
            ...
            # Если в деталях Resto нет адреса, пробуем взять его из заказа в БД
            # (Resto OLAP возвращает адрес одной строкой)
            ...
            if db_order and db_order.address_parts:
                # Объединяем: данные из OLAP (если есть) приоритетнее, но если в OLAP чего-то нет, берем из БД
    ...
    async def sync_zones_from_external_map(self, session: Session, url: Optional[str] = None) -> Dict[str, Any]:
        """
        Синхронизация внешних полигонов зон из облачного хранилища (Google Maps KML)
        """
    ...
    async def sync_zones_from_kml_file(self, session: Session, kml_content: str) -> Dict[str, Any]:
        ...
        for kz in kml_zones:
            ...
            if matched_zone:
                ...
                # Создание новой приоритетной зоны из KML файла
