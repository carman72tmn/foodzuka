...
class IikoService:
    ...
    async def get_delivery_restrictions(
        ...
        # Гарантируем, что результат всегда содержит ключ 'deliveryRestrictions'
        # iiko может вернуть как {"deliveryRestrictions": [...]}, так и просто список в некоторых версиях API
    ...
    async def fetch_and_parse_kml(self, url: str) -> List[Dict[str, Any]]:
        """
        Загружает KML файл по ссылке (например, Google Maps) и парсит его.
        Преобразует координаты в формат [[lat, lng], ...]
        """
        if not url:
            return []
            
        # Если ссылка на Google My Maps, пробуем получить KML версию
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                response.raise_for_status()
                # Принудительно используем UTF-8 или пробуем определить кодировку
                content = response.text
                logger.info(f"KML загружен, размер: {len(content)} байт. Начинаем парсинг...")
                return self.parse_kml_content(content)
            except Exception as e:
                logger.error(f"Ошибка загрузки KML по ссылке {url}: {e}")
                raise

    def parse_kml_content(self, kml_text: str) -> List[Dict[str, Any]]:
        """
        Парсит XML/KML содержимое и извлекает полигоны.
        Использует lxml с fallback на стандартный xml.etree.ElementTree.
        """
        import re
        import xml.etree.ElementTree as ET
        
        # Убираем мусор до <?xml или <kml
        kml_text = re.sub(r'^[^<]+', '', kml_text.strip())
        
        # Попытка через lxml (более надёжная)
        try:
            from lxml import etree
            parser = etree.XMLParser(recover=True, remove_blank_text=True)
            root = etree.fromstring(kml_text.encode('utf-8'), parser=parser)
            # Убираем ВСЕ namespace для упрощения XPath и поиска
            for el in root.iter():
                if isinstance(el.tag, str) and '}' in el.tag:
                    el.tag = el.tag.split('}', 1)[1]
                # Также чистим атрибуты с namespace
                attribs = {k.split('}', 1)[-1]: v for k, v in el.attrib.items()}
                el.attrib.clear()
                el.attrib.update(attribs)
            placemarks = root.xpath('.//Placemark')
        except (ImportError, Exception) as e:
            logger.warning(f"Ошибка lxml ({str(e)}), используем fallback xml.etree.ElementTree")
            # Удаляем namespace через regex перед парсингом для ElementTree
            kml_clean = re.sub(r'\s+xmlns[^"]*"[^"]*"', '', kml_text)
            # Убираем префиксы тегов <prefix:tag> -> <tag>
        ...
        zones = []
        logger.info(f"Найдено Placemark объектов: {len(placemarks)}")
        ...
        for pm in placemarks:
            ...
            name = (name_el.text or '').strip() if name_el is not None else 'Без имени'
            
            # Ищем координаты во всех вложенных элементах
            coord_text = ''
            for coord_el in pm.iter():
                # Игнорируем namespace в теге при сравнении
            ...
            if not coord_text:
                logger.warning(f"Placemark '{name}' — координаты не найдены")
                continue
            
            points = []
            # Координаты в KML разделены пробелами или переносами строк
            # Каждая точка в формате: lng,lat,alt
            ...
            if len(points) >= 3:  # Минимум 3 точки для полигона
                ...
                logger.info(f"Зона '{name}': {len(points)} точек")
            else:
                logger.warning(f"Зона '{name}': слишком мало точек ({len(points)}), пропуск")
...
�
    ...
    def parse_kml_content(self, kml_text: str) -> List[Dict[str, Any]]:
        """
        Парсит XML/KML содержимое и извлекает полигоны.
        Использует lxml для более надежного парсинга.
        """
        from lxml import etree
        import re
        
        try:
            # Превращаем в bytes для lxml если это строка, чтобы он сам разобрался с кодировкой в заголовке
            if isinstance(kml_text, str):
                # Если в начале есть мусор до <?xml, убираем его
                kml_text = re.sub(r'^[^<]+', '', kml_text)
                kml_bytes = kml_text.encode('utf-8')
            else:
                kml_bytes = kml_text

            # Используем парсер с восстановлением (recover=True), чтобы игнорировать мелкие ошибки XML
            parser = etree.XMLParser(recover=True, remove_blank_text=True)
            root = etree.fromstring(kml_bytes, parser=parser)
            
            # Удаляем неймспейсы для упрощения XPath
            ...
            for placemark in placemarks:
                ...
                # Ищем координаты во всех возможных местах используя local-name() 
                # для игнорирования неймспейсов (частая проблема KML)
                coord_nodes = placemark.xpath(".//*[local-name()='coordinates']/text()")
                
                if coord_nodes:
                    # Бывает несколько текстовых узлов в одном элементе или несколько полигонов
                    # Берем самый длинный (скорее всего это и есть полигон зоны)
                    coord_text = max(coord_nodes, key=len).strip()
                    points = []
                    
                    # Регулярка для извлечения координат: lng,lat[,alt]
                    # Ищем группы чисел, разделенные запятыми
                    # Формат: 37.123,55.123,0 или 37.123,55.123
                    ...
                    # Резервный способ через регулярку, если XPath не нашел (для странных форматов)
                    if not points:
                        # Ищем все цифры-запятые внутри текущего Placemark в текстовом виде
                        placemark_str = etree.tostring(placemark, encoding='unicode')
                        # Ищем длинные последовательности координат
                        coord_matches = re.findall(r'(-?\d+\.\d+,-?\d+\.\d+(?:,-?\d+\.\d+)?)', placemark_str)
                        if len(coord_matches) > 3: # Минимум 3 точки для полигона
                    ...
                    if points:
                        ...
                        logger.info(f"Зона '{name}' успешно распарсена: {len(points)} точек")
                    else:
                        # Логируем что именно было в Placemark для отладки
                        logger.warning(f"В Placemark '{name}' не найдено координат. XML: {etree.tostring(placemark, encoding='unicode')[:200]}...")
            
            return zones
        except Exception as e:
            logger.error(f"Критическая ошибка парсинга KML: {e}", exc_info=True)
            raise
