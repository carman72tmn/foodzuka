...
def get_template_variables():
    ...
    return {
        "variables": [
            {"name": "order.external_number", "description": "Номер заказа (из iiko)"},
            {"name": "order.total_amount", "description": "Сумма без скидки"},
            {"name": "order.total_with_discount", "description": "Итоговая сумма (к оплате)"},
            {"name": "order.final_price", "description": "Итоговая сумма (к оплате) - синоним"},
            {"name": "order.total_discount", "description": "Сумма скидки"},
            {"name": "order.discount_sum", "description": "Сумма скидки - синоним"},
            {"name": "order.items_summary", "description": "Состав заказа с ценами (полный)"},
            {"name": "order.customer_name", "description": "Имя гостя"},
            {"name": "order.customer_phone", "description": "Телефон гостя"},
            {"name": "order.address", "description": "Адрес доставки"},
            {"name": "order.delivery_time", "description": "Время доставки (ожидаемое)"},
            {"name": "order.comment", "description": "Комментарий к заказу"},
            {"name": "order.courier_name", "description": "Имя курьера"},
            {"name": "order.map_yandex", "description": "Ссылка на Яндекс.Карты"},
            {"name": "order.map_2gis", "description": "Ссылка на 2ГИС"},
        ],
        "events": [
...
def get_logs(limit: int = 1000, db: Session = Depends(get_db)):
    # Увеличили лимит по умолчанию до 1000 для удобства, 
