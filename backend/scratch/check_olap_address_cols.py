...
async def main():
    ...
    async with httpx.AsyncClient(verify=False, timeout=30) as client:
        ...
        if r.status_code == 200:
            resp_data = r.json()
            # iiko usually returns a dict with 'columns' key or similar in v2
            print(f"Full response keys: {list(resp_data.keys()) if isinstance(resp_data, dict) else 'not a dict'}")
            ...
            address_cols = [c["id"] for c in cols if isinstance(c, dict) and any(x in c.get("id", "") for x in ["Address", "City", "Street", "House", "Flat", "Entrance", "Floor", "Doorphone"])]
            print(f"Address related columns: {json.dumps(address_cols, indent=2, ensure_ascii=False)}")
        else:
