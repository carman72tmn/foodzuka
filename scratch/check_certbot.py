...
def check_certbot_logs(ip, pwd):
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        client.connect(ip, username="root", password=pwd, timeout=30)
        
        print("--- Docker PS (Certbot) ---")
        stdin, stdout, stderr = client.exec_command("docker ps -a | grep certbot")
...
if __name__ == "__main__":
    check_certbot_logs("178.212.13.48", "12101991Qq")
