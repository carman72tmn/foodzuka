...
const settings = ref({
  ...
  external_menu_id: "",
  terminal_group_id: "",
  price_category_id: "",  // ИСПРАВЛЕНИЕ: поле отсутствовало — сбрасывалось при каждом сохранении
  payment_type_cash: "",
  payment_type_card: "",
  payment_type_online: "",
  payment_type_bonus: "",
...
const priceCategories = ref([]);  // ИСПРАВЛЕНИЕ: добавлен справочник категорий цен
...
const saveSettings = async () => {
  saving.value = true;
  try {
    const res = await fetch(`${API_BASE}/settings/`, {  // ИСПРАВЛЕНИЕ: добавлен trailing slash (без него nginx делал 307 redirect с потерей тела запроса)
      method: "POST",
...
const loadPriceCategories = async () => {
  try {
    ...
    if (res.ok) {
      const data = await res.json();
      priceCategories.value = data.map((c) => ({
        title: c.name || c.id,
        value: c.id,
      }));
    }
...
const loadAllReferences = async () => {
  await Promise.all([
    ...
    loadExternalMenus(),
    loadDiscountTypes(),
    loadPriceCategories(),  // ИСПРАВЛЕНИЕ: добавлена загрузка категорий цен
  ]);
};
...
class="me-2" />
                ...
                <VCardText>
                  ...
                  <VTextField
                    ...
                    hint="Используется для фильтрации и часового пояса"
                    persistent-hint
            ...
            <VCol cols="12" md="6">
              <VCard>
                <VCardTitle>
                  <VIcon icon="mdi-map-marker-distance" class="me-2" />
                  Формат адреса iiko
                </VCardTitle>
                <VCardText>
                  <VSelect
                    ...
                    :items="[
                      { title: 'Классический (из компонентов: улица, дом...)', value: 'components' },
                      { title: 'Строка (из поля line1)', value: 'line1' }
                    ]"
                    label="Формат парсинга"
                    hint="Как собирать адрес из данных iiko Cloud"
                    persistent-hint
                  />
                </VCardText>
              </VCard>
...
class="me-2" />
            ...
            <VCol cols="12" md="6">
              <VCard>
                <VCardTitle>
                  <VIcon icon="mdi-tag-multiple" class="me-2" />
                  Категория цен
                </VCardTitle>
                <VCardText>
                  <VSelect
                    ...
                    label="Категория цен"
                    hint="Категория цен для выгрузки меню из iiko"
                    persistent-hint
                    clearable
                  />
                </VCardText>
