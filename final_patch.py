                ...
                if product:
                    ...
                    if size_prices:
                        ...
                        for sp in size_prices:
                            size_id = sp.get("sizeId") or "default"
                            size = session.exec(select(ProductSize).where(ProductSize.product_id == product.id, ProductSize.iiko_id == size_id)).first()
...
if __name__ == '__main__':
    ...
    # It starts with '    async def sync_prices' and ends before the next '    async def sync_stop_lists'
    # We use a broad match for the method body
    start_marker = '    async def sync_prices(self, session: Session) -> Dict[str, Any]:'
    end_marker = '    async def sync_stop_lists'
    
    if start_marker in content and end_marker in content:
        parts = content.split(start_marker)
        prefix = parts[0]
        remainder = parts[1].split(end_marker)
        suffix = end_marker + remainder[1]
        
        new_content = prefix + NEW_SYNC_PRICES_BLOCK + suffix
        
        with open(sync_service_path, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Price sync successfully updated to v2 logic.")
    else:
        print("Markers not found, could not patch.")
